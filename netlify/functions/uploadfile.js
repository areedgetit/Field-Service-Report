...
exports.handler = async function (event, context) {
  ...
  try {
    ...
    try { tokenData = JSON.parse(tokenText); } 
      ...
      return {
        statusCode: 500,
        body: JSON.stringify({ message: "Invalid token response from Microsoft", raw: tokenText }),
      };
    }
    ...
    if (!tokenResponse.ok || !tokenData.access_token) {
      ...
      return {
        ...
        body: JSON.stringify({ message: "Failed to obtain access token", response: tokenData }),
      };
    }
    ...
    if (event.isBase64Encoded) {
      ...
      console.log("Decoded base64 fileContent length:", fileContent.length);
    } else {
      fileContent = Buffer.from(event.body, "utf-8");
      console.log("Raw utf-8 fileContent length:", fileContent.length);
    }
    ...
    if (!uploadResponse.ok) {
      ...
      return {
        ...
        body: JSON.stringify({ message: "Upload failed", status: uploadResponse.status, response: uploadResult }),
      };
    }
